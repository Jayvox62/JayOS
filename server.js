/**
 * JayOS — single-file Express server
 *
 * Endpoints:
...
function safeRequire(name) {
  try {
    ...
    if (err.code === "MODULE_NOT_FOUND" && err.message.includes(name)) {
      ...
      console.error("");
      console.error("  1. Container Manager → Project → jamieos → Build, then");
      ...
      console.error("       cd /volume1/docker/jamieos");
      console.error("       sudo docker compose build --no-cache");
      console.error("       sudo docker compose up -d");
      console.error("");
      console.error("  3. Container Manager → Image → delete the jamieos image");
      console.error("     entirely → then Project → jamieos → Build (forces a");
...
const VERSION = "1.1.0"; // bump this when you change the code so the UI reflects it
...
try {
  ...
  console.error("       Quickest workaround: `chmod -R 777 /volume1/docker/jamieos/data`");
  ...
  certifications: [
    {
      ...
      vendor: "Degree",
      short: "BSc",
      full: "BSc in Computing",
      desc: "Degree focused on software development, computing systems, networking, and problem solving.",
      inProgress: false,
    },
    {
      id: "cert-1",
      ...
      vendor: "ITIL",
      short: "ITIL",
      full: "ITIL Foundation",
      desc: "Pending certification. Curriculum completed.",
      inProgress: true,
    },
  ],
...
const server = app.listen(PORT, () => {
  console.log(`JayOS v${VERSION} listening on :${PORT}`);
