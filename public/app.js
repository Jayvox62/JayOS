/* =========================================================
   JayOS front-end — vanilla JS, no build step.
...
const CLIENT_BUILD = "JayOS app.js v1.1.0";
console.log("JayOS client build:", CLIENT_BUILD);
...
async function fetchVersion() {
  try {
    ...
    console.log(
      `%cJayOS%c  client loaded, server v${state.serverVersion.version} started ${state.serverVersion.started}`,
  ...
  const run = (raw) => {
    ...
    switch (base) {
      case "help":
        ...
        break;
      case "whoami":
        print(`${state.settings.identity.username}${state.isAdmin ? "  [admin]" : ""}`);
        break;
      case "about":
      case "bio":
